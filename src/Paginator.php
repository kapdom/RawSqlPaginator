...
class Paginator
{
    ...
    {
        $this->domain = url('/');
        $this->getPageFromRequestAndSetUri($request);
    }
...
PAGINATION;
     ...
     * @return null|string
     */
    protected function disableIfFirstOrLast(bool $first = false): ?string
    {
        if($first){
     ...
     * @throws RawPaginatorException
     */
    private function getPageFromRequestAndSetUri(Request $request) :void
    {
        $pageAsGetParam = $request->request->get('page');
        if ($pageAsGetParam !== null) {
            $uriWithPlaceholder = str_replace('page='.$pageAsGetParam,'page=%',$request->getRequestUri());
            $this->currentPage = (int)$pageAsGetParam;
            $this->fullUriPath = $this->domain.$uriWithPlaceholder;

            return;
        }

        $pathArray = explode('/',$request->path());
        $pageArrayPosition = array_search('page', $pathArray, true);
        if ($pageArrayPosition !== false
            && isset($pathArray[$pageArrayPosition+1])
            && is_numeric($pathArray[$pageArrayPosition+1])
        ){
            $this->currentPage = (int)$pathArray[$pageArrayPosition+1];
            $pathArray[$pageArrayPosition+1] = '%';
        ...
        if (count($numericValueKey) === 1) {
            $this->currentPage = $pathArray[$numericValueKey[0]];
            $pathArray[$numericValueKey[0]] = '%';
            $this->fullUriPath = $this->domain.'/'.implode('/', $pathArray);
        } else {
            throw new RawPaginatorException('Unknow page value postion in url.');
        }
    }
